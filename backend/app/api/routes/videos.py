...
async def upload_video(
    ...
    if file.content_type != settings.ALLOWED_VIDEO_MIME:
        raise HTTPException(
            status_code=415,
            detail=translator.t('errors.bad_mime', locale=locale),
        )
    ...
    if size_mb > settings.MAX_VIDEO_MB:
        raise HTTPException(
            status_code=413,
            detail=translator.t('errors.file_too_large', locale=locale),
        )
    character = None
    if character_id is not None:
        ...
        if not character:
            raise HTTPException(
                status_code=404,
                detail=translator.t('characters.not_found', locale=locale),
            )
    ...
    db.commit()
    db.refresh(video)
    added, bonus = award_for_video(
        db, user, first_bonus=settings.FIRST_UPLOAD_BONUS
    )
    return UploadResult(
