...
def register(
    request: Request, data: UserCreate, db: Session = Depends(get_db)
):
    locale = getattr(request.state, 'locale', settings.DEFAULT_LOCALE)
    existing = db.query(User).filter(User.email == data.email).first()
    if existing:
        raise HTTPException(
            status_code=400, detail=translator.t('auth.exists', locale=locale)
        )
...
def login(request: Request, data: UserCreate, db: Session = Depends(get_db)):
    ...
    if not user or not verify_password(data.password, user.hashed_password):
        raise HTTPException(
            status_code=401, detail=translator.t('errors.auth', locale=locale)
        )
...
def promote_user(
    user_id: int, request: Request, user=Depends(get_current_user)
):
    ...
    raise HTTPException(
        status_code=501,
        detail=translator.t('admin.not_implemented', locale=locale),
    )
